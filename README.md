...
- Adds opt-in **aHQ (experimental)** beside the fixed HQ workflow. aHQ works with no animation loaded: it resolves the selected, or uniquely visible, grouped LiDAR scene and prepares a 20%-guarded fine cache around the current camera. A loaded animation is not required and does not bound navigation; moving along it advances the same guarded camera cache. Each 1 mm role builds one machine-local Morton-ordered, 1--8 MiB spatial-block cache under `Saved/.invisible_places/cache/adaptive_hq/`: hot position/colour/normal/source-ID bytes are separate from field-major scalar bytes, so camera requests seek only intersecting blocks and currently used columns. Active blocks always remain decoded and recently visited inactive blocks use a 6 GiB-per-role RAM LRU, making orbit returns and back-and-forth scrubbing reuse the warm working set instead of reopening it. Source size, timestamp, schema, and sampled-content identity validate the disk cache automatically. The canonical/export PLY is never rewritten or cached in OneDrive. The complete 5 mm scene remains CPU/GPU resident as fallback and has its own hot-geometry/per-field cache; on a valid cache hit startup reads those local binaries rather than parsing the full PLY. Inside a valid guard, a camera-stable GPU hash blends fine and 5 mm density across projected depth. Crossing a guard restores complete 5 mm for new areas but retains the previous depth-faded fine patch until the replacement is ready, instead of visibly clearing all HQ points. Fixed HQ remains available, and aHQ remains session-only and excluded from exports and serialization.
