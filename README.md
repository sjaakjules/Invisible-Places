...
- Adds an independent session-only **HQ** control beside Seam/A/B. The active linked pair prepares in the background while the button shows monotonic stage progress. HQ off is a complete 5 mm ROCK/SAND/VEG view. HQ on keeps complete 5 mm SAND, uses compact 1 mm ROCK/VEG patches inside the union of A's and B's authored `0.5` camera frusta with a 5% border on every viewport side, and masks the corresponding 5 mm points only inside that union. The two patches and complementary masks publish atomically, remain ready for instant Seam/A/B switching, and never change the playhead, camera, playback, saved Visible Point Cloud, or project/animation schema. Full still, frame-preview, video, and EXR exports continue to load the canonical complete 1 mm ROCK/SAND/VEG bundle; HQ never opens 1 mm SAND.
