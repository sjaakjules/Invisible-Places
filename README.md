...
- **Fast render iteration:** the app has deterministic point budgets, fixed-density animation-section frustum masks, cached camera-path evaluation, and a Fast Basic point-render path for keeping navigation, playback, and look-development responsive.
...
- Keeps the selected display density stable during navigation and playback. Focusing an unlinked animation's Feature Run View prepares one conservative full-density index mask in the background for that section, while Preview Performance Mode exposes whether opaque-styled procedural layers are actually using depth-tested accumulation.
...
- Makes **Export Current**, **Render in Background**, and their active-queue forms ask whether to export linked member **A**, **B**, or **Both**, independently of which member file is loaded or currently presented in Seam/A/B view. A and B use the same stable path-sorted identity as the transport; Both freezes two member-specific render setups at the click and serializes them A then B in both foreground and detached queues. Each setup records the chosen animation file and edited state, so member-specific animation/run settings resolve against the intended path without changing the shared playhead or view mode.
- Adds an independent session-only **HQ** control for both unlinked animations and reciprocal pairs. Preparation runs in the background while the button shows monotonic stage progress. A pair retains the compact union of A's and B's authored `0.5` camera frusta; an unlinked animation instead uses nine evenly spaced views from start through end plus every authored camera-key view, so its higher-density patch spans the complete shot. Every view has a 5% border on each viewport side. HQ off is a complete 5 mm ROCK/SAND/VEG view. HQ on uses compact 1 mm ROCK/VEG inside the applicable view union and masks the corresponding 5 mm points only there. A project-saved **Sand** toggle beside the density selector is off by default; enabling it adds 1 mm SAND to that same patch/mask union, with complementary 5 mm SAND outside, while preserving Sand Visual, Shoreline, Timing, Rain, Flow, and Seepage behavior. The 1, 2, or 3 mm patch-spacing selector keeps every scanned point or thins each included role with the display cache's density-preserving cell stratification (one centred parent per 2 mm or 3 mm cell, hash-dithered quotas, matching density compensation), so a lighter patch reads like the 1 mm patch rather than speckled. All requested two or three patches and complementary masks publish atomically and never change the playhead, camera, playback, or saved Visible Point Cloud. Full still, frame-preview, video, and EXR exports continue to load the canonical complete 1 mm ROCK/SAND/VEG bundle; with Sand off, HQ never fingerprints or opens the large 1 mm SAND source.
- Adds opt-in **aHQ (experimental)** beside the fixed HQ workflow. aHQ works with no animation loaded: it resolves the selected, or uniquely visible, grouped LiDAR scene and prepares a 20%-guarded fine cache around the current camera. A loaded animation is not required and does not bound navigation; playback and scrubbing advance the same guarded camera cache without preloading the whole path. Each 1 mm role builds one machine-local Morton-ordered, 1--8 MiB spatial-block cache under `Saved/.invisible_places/cache/adaptive_hq/`: hot position/colour/normal/source-ID bytes are separate from field-major scalar bytes, so camera requests seek only intersecting blocks and currently used columns. Decoded blocks are subdivided into conservative 128-point Morton micro-blocks in memory, avoiding millions of exact CPU frustum tests without changing the persistent cache format. Active blocks always remain decoded and a distance/recency-aware 6 GiB-per-role fringe retains nearby or recently scrubbed data. Source size, timestamp, schema, and sampled-content identity validate the disk cache automatically. The canonical/export PLY is never rewritten or cached in OneDrive. The complete cached 5 mm scene remains CPU/GPU resident as fallback; a reusable grid-cell lookup builds guard masks, bounded coarse-mask history covers the movement fringe, and GPU index buffers keep their capacity between refreshes. Crossing a guard restores complete 5 mm for new areas while the previous depth-faded fine patch remains until replacement publication; superseded CPU patches retire asynchronously after navigation- or timeline-specific grace periods, with a 4 GiB pressure cap. Fixed HQ remains available, and aHQ remains session-only and excluded from exports and serialization.
...
- Can freeze the current animation or every selected batch animation, Timing Take, water, visual, export preset, and click-time render viewport into immutable background-render packages. Batch packages run sequentially. When a foreground or detached export is already running, Current Render Setup and Batch Export expose explicit **Queue to Active Export** and **Queue to Background Export** actions. Detached followers wait without opening Vulkan or loading the scene, survive closing or rebuilding the editor, and begin after their predecessor status becomes terminal. Each low-priority worker has a compact monitor showing preparation, frame progress, elapsed time, ETA, output path, and graceful cancellation; closing only the monitor hides it while rendering continues. Active workers limit their writer queue to one frame and deliberately pause between GPU steps; they still share GPU memory and bandwidth with any reopened editor. Persistent status files can also attach read-only progress windows to jobs launched by an older build.
...
- Only the authoritative grouped scene is resident: switching between Fossils and Pools releases the outgoing scene's direct-folder CPU/GPU clouds and queued loads before the incoming scene publishes. Saved projects likewise restore at most one available local scene as loaded.
- Delimiter-bounded `WATER` files are density-aware auxiliary variants rather than independent overlays. Live display selects one exact/nearest WATER density (5 mm base uses WATER 5 mm); final output selects the finest terrain bundle and nearest WATER density (Site1 uses 1 mm terrain plus WATER 2 mm). Recovery copies containing `old`, `backup`, or `archive` are ignored, and measured family compensation keeps point size, opacity, and emission consistent across WATER densities.
...
- Linked-density validation uses `--gui-smoke linked-hq-sample-scene` for the cancellable SampleScene preparation and `--gui-smoke linked-hq-scene3` for the production-source diagnostic. Both pin the default-off Sand mode: 1 mm SAND remains untouched, both ROCK/VEG patches publish atomically, Seam/A/B retain HQ, role Visual/effect state remains matched, and export source selection remains the complete canonical 1 mm bundle.
- `--gui-smoke linked-hq-frame-capture` is the short form of the timing smoke: it prepares HQ, renders HQ off/on at each member's 0.5 camera, and writes one PPM per phase so patch density and compensation can be compared by eye and by luminance/high-pass metrics.
- `--gui-smoke linked-hq-frame-timing` profiles the HQ live view on the authored workspace project's active reciprocal pair (or `--smoke-project`): it waits for the patches, then samples wall-clock, CPU and GPU phase timings at each member's normalized 0.5 camera with HQ off/on, one water feature ablated at a time, and a backwards-facing camera that frustum-culls every patch point. The JSON report records the patch/remainder point counts and per-phase draw counts so HQ regressions can be separated from effect cost.
