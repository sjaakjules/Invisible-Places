...
- **Portable project state:** schema-76 project JSON, schema-23 animation JSON, and schema-30 standalone water-source JSON store authored scenes, explicit active water-scene ownership, animation, styles, water controls (including overlap-safe clip-key ownership), live-view window preferences, and compact cache manifests; derived shared-surface and settled Flow-path payloads live in validated sidecars rather than bloating the project file.
...
- Saves and reloads schema-76 project JSON containing authoritative scene density groups, explicit active water-scene ownership, point-cloud styles, camera shots, animation paths, saved visuals, the live-view window preference and lock, water state/cache manifests, water timing settings clips with explicit per-key ownership, manual spline handles, and export selections. Animation documents use schema 23 and standalone water-source documents use schema 30.
