...
- Adds an independent session-only **HQ** control beside Seam/A/B. The active linked pair prepares in the background while the button shows monotonic stage progress. HQ off is a complete 5 mm ROCK/SAND/VEG view. HQ on keeps complete 5 mm SAND, uses compact 1 mm ROCK/VEG patches inside the union of A's and B's authored `0.5` camera frusta with a 5% border on every viewport side, and masks the corresponding 5 mm points only inside that union. A patch-spacing selector beside the button (1, 2 or 3 mm, saved with the project) keeps every scanned 1 mm point or thins them with the display cache's density-preserving cell stratification (one centred parent per 2 mm or 3 mm cell, hash-dithered quotas, matching density compensation), so a lighter patch reads like the 1 mm patch rather than speckled; 2 mm is near-indistinguishable on rock and vegetation, 3 mm visibly thins fine vegetation. The two patches and complementary masks publish atomically, remain ready for instant Seam/A/B switching, and never change the playhead, camera, playback, saved Visible Point Cloud, or project/animation schema. Full still, frame-preview, video, and EXR exports continue to load the canonical complete 1 mm ROCK/SAND/VEG bundle; HQ never opens 1 mm SAND.
...
- Linked-density validation uses `--gui-smoke linked-hq-sample-scene` for the cancellable SampleScene preparation and `--gui-smoke linked-hq-scene3` for the production-source diagnostic. Both verify that 1 mm SAND remains untouched during HQ preparation, both ROCK/VEG patches publish atomically, Seam/A/B retain HQ, role Visual/effect state remains matched, and export source selection remains the complete canonical 1 mm bundle.
