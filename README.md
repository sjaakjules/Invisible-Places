...
- **Two-computer OneDrive workspace:** set `INVISIBLE_PLACES_WORKSPACE_DIR` for authored JSON and `INVISIBLE_PLACES_SHARED_DATA_DIR` for source PLYs, or use the two Project-panel folder controls. The shared source folder contains only production Scene3 ROCK/SAND/VEG at 1 mm and 5 mm plus `Site3-MESH.ply` and `Site3-MESHSampled-5mm.ply`. Project/animation paths remain portable through `@data`, `@workspace`, and `@local-renders`; every cache, render, render-history index, validation artefact, and build stays machine-local. Save records the exact bytes loaded and refuses a stale project/animation overwrite if OneDrive has delivered another computer's edit; a change arriving during staging also preserves the outgoing JSON under local `Saved/conflict_recovery/`. OneDrive cannot supply an offline distributed lock, so intentional parallel work on the same file still uses distinct Save As names before later review.
...
- Keeps that hard-split preview available after Apply through **Seamed View** on every valid linked pair. Scrubbing or playing either member resolves the exact reciprocal frame inside its saved start/end overlaps, follows the captured feature anchors for the active session (with an overlap-progress fallback for reloaded links), and automatically keeps the ending animation left and the starting animation right. Each physical seam retains its own adjustable pixel offset when A/B are swapped; outside an overlap the viewport returns to the ordinary current animation.
- Places **Linked A/B View** and **Seamed View** above Global Animation Position. A linked pair can share one Timing Take over its complete unique loop: the full-loop editor authors Rain, Water, and visual-setting keys once, while either ordinary animation view projects those keys into its own local camera window. Cyclic virtual neighbours preserve the authored interpolation and derivative across loop zero and across either path boundary—no artificial endpoint key or Smooth-Step slowdown is introduced. Global Animation Position still moves only the camera; procedural waves, Rain, and trails continue on their independent live/export clocks.
...
- Saves and reloads schema-77 project JSON containing authoritative scene density groups, explicit active water-scene ownership, point-cloud styles, camera shots, animation paths, saved visuals, the live-view window preference and lock, water state/cache manifests, shared linked-loop Timing Takes, water timing settings clips with explicit per-key ownership, manual spline handles, and export selections. Animation documents use schema 24 and standalone water-source documents use schema 30.
