...
float WaterTrailFade(uint pointIndex) {
    if (WaterStreamOverlayEnabled()) {
        const float age = clamp(LoadScalarFieldValueForPoint(kWaterStreamPointAgeFieldSlot, pointIndex), 0.0, 1.0);
...
vec2 ApplyWaterFlowAnimation(float opacity, float emissive, uint pointIndex) {
    ...
    if (WaterStreamOverlayEnabled()) {
        const float age = clamp(LoadScalarFieldValueForPoint(kWaterStreamPointAgeFieldSlot, pointIndex), 0.0, 1.0);
        const float speed = max(0.001, LoadScalarFieldValueForPoint(kWaterStreamSpeedFieldSlot, pointIndex));
        const float confidence = clamp(LoadScalarFieldValueForPoint(kWaterStreamConfidenceFieldSlot, pointIndex), 0.0, 1.0);
        const float wetness = clamp(LoadScalarFieldValueForPoint(kWaterStreamWetnessFieldSlot, pointIndex), 0.0, 1.0);
        const float seed = LoadScalarFieldValueForPoint(kWaterStreamPointSeedFieldSlot, pointIndex);
