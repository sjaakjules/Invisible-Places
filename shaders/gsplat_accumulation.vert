...
void main() {
    const uint splatIndex = uint(gl_InstanceIndex);
    const vec2 corner = CornerPosition(uint(gl_VertexIndex));

    const vec3 localCenter = centers.values[splatIndex].xyz;
    const float footprintBoost = max(styleData.extra.x, 1e-3);
    const vec3 decodedScale = max(scales.values[splatIndex].xyz * styleData.style.y, vec3(1e-4));
    const vec4 rotation = rotations.values[splatIndex];
    const float opacity = opacities.values[splatIndex];

    const mat4 localToWorld = styleData.control.z != 0u ? styleData.localToWorld : mat4(1.0);
