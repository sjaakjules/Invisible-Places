...
- Animation paths derived from camera paths, editable camera/focus keys, playback/scrubbing, save/load, focus distance, aperture metadata, LiDAR associations, a default live-view window size, and per-animation Quick MP4 export settings/visual selections in `.ipanim.json` schema v22.
- Reciprocal pan extension for target-driven, fixed-lens paths: editable working-scene triangles define the existing A-start/B-end and B-start/A-end 50%-blend midpoints. Each inward seam span generates both a source pre-roll and partner tail, using two keys at each simple end or three for a crossed key/inflection, plus localized first/last endpoint alignment. Signed fitted-seam review includes A/B switching and a real two-camera hard split whose adjustable boundary follows the projected anchors; water work is suppressed while the assistant is active. Final Preview exposes a bounded, per-key transition smoother: generated keys, both coordinated 50% midpoint pairs, and up to two inward authored neighbours are recommended. Each midpoint pair moves through a shared triangle-derived transform and must retain its three-node projected alignment while the objective reduces signed screen-velocity and rotation-rate roughness. Apply creates a reciprocal blend link for an unlinked pair or updates its existing pair. Schema v22 stores explicit correction tangents and a deprecated migration marker. Current extensions affinely shift/rescale normalized timing/water/effect keys so they remain attached to the same old camera poses. Camera position selects those keyed values but never drives procedural motion: live effects use steady elapsed time even with a stationary camera, and export uses deterministic full output time.
