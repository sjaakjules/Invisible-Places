...
- Reciprocal pan extension for target-driven, fixed-lens paths: editable common-scene triangles define the existing A-start/B-end and B-start/A-end 50%-blend poses. Each inward source span produces an immutable partner candidate with two generated tail keys, or three when the span crosses a key or inflection, plus a localized former-terminal alignment. Apply creates a reciprocal blend link for an unlinked selected pair or updates its existing pair. Schema v22 stores explicit correction tangents and a deprecated migration marker for candidates written by the initial extension implementation. Current extensions rescale every normalized timing/water/effect key from the old frame count onto the new one, preserving its absolute 30 fps frame. Camera position selects those keyed values but never drives procedural motion: live effects use steady elapsed time even with a stationary camera, and export uses deterministic full output time.
