...
- Reciprocal pan extension for target-driven, fixed-lens paths: editable working-scene triangles define the existing A-start/B-end and B-start/A-end 50%-blend midpoints. Each inward seam span generates both a source pre-roll and partner tail, using two keys at each simple end or three for a crossed key/inflection, plus localized first/last endpoint alignment. Signed fitted-seam review includes A/B switching and a real two-camera hard split whose adjustable boundary follows the projected anchors; water work is suppressed while the assistant is active. Final Preview exposes a bounded, per-key transition smoother: the exact triangle-aligned midpoint keys stay locked, generated keys and up to two inward authored neighbours are recommended, and the user may include or exclude eligible keys before reducing screen-flow and rotation roughness. Apply creates a reciprocal blend link for an unlinked pair or updates its existing pair. Schema v22 stores explicit correction tangents and a deprecated migration marker. Current extensions affinely shift/rescale normalized timing/water/effect keys so they remain attached to the same old camera poses. Camera position selects those keyed values but never drives procedural motion: live effects use steady elapsed time even with a stationary camera, and export uses deterministic full output time.
