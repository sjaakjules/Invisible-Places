...
- Camera shot save/load, ordered camera paths, quaternion interpolation, 30 fps timing, CPU-assisted surface pivot picking, focus-key world-Z orbit handles, detached hover-expanding XY/ZX/ZY whole-rig translation handles, and transactional focus-node Live Edit Mode with full camera/lens capture.
- Animation paths derived from camera paths, editable camera/focus keys, playback/scrubbing, save/load, focus distance, aperture metadata, LiDAR associations, a default live-view window size, per-animation Quick MP4 export settings/visual selections, and an unlinked preferred Blend Partner in `.ipanim.json` schema v23.
- Matching-Frame Key Alignment keeps **Fix A+B Lens** beside **Extend Both Seams…**. Its ordinary paired timelines support stable right-click key selection plus a camera-alignment clipboard for focus-relative world-Z orbit angle, elevation, distance, and authored horizon/roll. The standalone repair creates paired edited paths with one median fixed-lens profile per animation and a conservative pair-wide clip union; the extension wizard can instead hold that repair privately until Apply or Cancel.
