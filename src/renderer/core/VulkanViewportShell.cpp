...
void VulkanViewportShell::ReadPreviousGpuCompactionResults(std::size_t frameIndex) {
    ...
    if (checked) {
        ...
        diagnostics_.adaptiveGpuCompactionParityStatus =
            passed ? "passed previous-frame prefix count/source-fingerprint/checksum"
                   : "mismatch in previous-frame prefix count/source-fingerprint/checksum";
    }
...
bool VulkanViewportShell::RecordGpuDrawItemCompactionForScene(
    ...
    if (recordedAny) {
        ...
        diagnostics_.adaptiveSelectionFallbackReason =
            ...
            "CPU-selected renderer-profile-filtered performance-clamped represented-count-limited depth-windowed rank-limited projected-footprint feature draw items are prefix-filtered, compacted, checksummed, and converted to a diagnostic indirect command by compute for comparison; "
