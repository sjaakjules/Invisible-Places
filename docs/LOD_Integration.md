...
diagnostic viewport draws, and compute-generated indirect command output for
eligible viewport draws. GPU compute selection remains guarded behind feature,
parity, and timing proof; on the local MoltenVK runtime the current reported path is
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  rank/class predicate, compacted, and checksummed by a compare-only compute pass,
  and current viewport indirect commands can be generated by a compute dispatch
  from the CPU-selected compact draw-item count, so selection semantics remain
  unchanged. EXR command buffers and
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  representatives, compacts, and checksums CPU-selected draw items, while
  another compute shader writes the indirect command before the render pass. CPU
  traversal remains authoritative because full CPU/GPU compute-selection parity
  and timing are not yet available. Fast Basic recorded 1 GPU
  ...
  0, compacted up to 5,578 items at 1.86154 ms, and matched previous-frame
  CPU/GPU count 3,886 with checksum 1,674,177,483, plus 1 GPU indirect-command
  dispatch at 0.526583 ms. Beauty stress recorded 1 GPU performance-clamped
  ...
  items at 2.12917 ms, and matched
  previous-frame CPU/GPU count 2,327 with checksum 1,862,844,103. Coverage ratio
