...
prefix-selection/performance-clamped represented-count-limited projected-footprint depth-windowed rank-and-feature-class filtering/compaction/source-fingerprint/checksum diagnostics for
diagnostic viewport draws, diagnostic compacted-count indirect command output,
bounded prefix-range dispatch, and CPU-count compute-generated submitted indirect
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  depth-windowed rank/class predicate, dispatched over the bounded prefix range,
  compacted with source-identity fingerprints, checksummed, and converted into a diagnostic
...
- `lod_compare_metrics.json` schema v3 reports source/cache fingerprints,
  ...
  profile-mask/opacity-window/emission-window/frustum enabled/guard/fallback/position-count fields,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  build, focused GPU/LOD tests, full CTest 189/189, stream check, and repeat
  `--lod-compare` runs. The local MoltenVK runtime reported GPU-selection feature
  ...
  representatives, dispatches only the bounded prefix range, compacts with
  source-identity fingerprints, checksums CPU-selected draw items, and converts
  ...
  metadata-only prefix compaction. The latest source-fingerprint run records CPU reference timing
  ...
  compacted up to 4,626 items, measured 0.749208 ms for the CPU reference
  predicate and 3.096 ms for GPU compaction, matched previous-frame CPU/GPU
  count 3,139, source fingerprint 98,251,190 / 98,251,190, and checksum
  ...
  3.30625 ms. Beauty stress recorded 1 matching
  ...
  0.559375 ms for the CPU reference predicate and 2.46263 ms for GPU compaction, matched
  previous-frame CPU/GPU count 2,301, source fingerprint 1,678,025,600 / 1,678,025,600,
  and checksum 933,328,382 / 933,328,382, and matched
  compacted indirect CPU/GPU vertices 2,301 / 2,301; Beauty GPU indirect-command
  timing was 5.82862 ms. The prior frustum shader measurement was
  ...
  fallback or budget exceedance. CPU fallback plus CPU-count submitted indirect
  command remain active. The stream
