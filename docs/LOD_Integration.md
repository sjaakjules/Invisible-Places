...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  compacted, checksummed, and converted into a diagnostic
...
- `lod_compare_metrics.json` schema v3 reports source/cache fingerprints,
  ...
  culling fields, GPU prefix-compaction input/dispatched/selection-limit fields,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  representatives, dispatches only the bounded prefix range, compacts,
  ...
  metadata-only prefix compaction. The latest bounded-dispatch run records CPU reference timing
  ...
  compacted up to 4,626 items, measured 0.522875 ms for the CPU reference
  predicate and 1.644 ms for GPU compaction, matched previous-frame CPU/GPU
  ...
  3.15692 ms. Beauty stress recorded 1 matching
  ...
  0.528834 ms for the CPU reference predicate and 3.18675 ms for GPU compaction, matched
  ...
  timing was 0.1275 ms. The prior frustum shader measurement was
