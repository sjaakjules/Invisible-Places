...
draw items, ordered draw-index-preserving output writes when the fitted CPU-selected range is complete, order-sensitive output identity parity, compare-only protected feature-class, stable-rank-prefix, projected-area, represented-count, and coverage-compensation predicate probes, compacted-output descriptor/barrier submission after parity and performance gates pass, diagnostic compacted-count indirect
command output, full CPU-selected-range dispatch, and CPU-count compute-generated submitted indirect
command output only when compacted output is not eligible for the current image. It also tracks CPU/GPU full-range, protected feature-class, stable-rank-prefix, projected-area, represented-count, and coverage-compensation predicate
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  submitting the reduced output. A
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  output identity 2,876,771 / 2,876,771. Fast Basic measured 227.852 ms for the
  CPU reference predicate and 11.0021 ms for GPU full-range compaction, so the
  ...
  checksum 3,534,349,174, and measured 8.75279 ms CPU reference vs 0.080291 ms
  ...
  checksum 3,671,843,879, and measured 58.5389 ms CPU reference vs 0.032084 ms
  GPU. A projected-area GPU probe filtered `footprintAreaPixels >= 4`, matched
  CPU/GPU count 102,497 / 102,497, source fingerprint 263,410,465,
  checksum 65,403,358, and measured 28.8078 ms CPU reference vs 0.027375 ms
  ...
  checksum 1,624,835,333, and measured 89.5583 ms CPU reference vs 0.025166 ms
  ...
  measured 84.5029 ms CPU reference vs 0.215334 ms GPU. The
  ...
  as a first-frame fallback only, with 1 dispatch at 0.010625 ms in the latest
  sample run. Beauty stress kept max GPU point pass 0.1145 ms with EWMA
  0.0370978 ms and reported no adaptive/full-source fallback or budget
  ...
  projected-area, represented-count, and coverage-compensation filtering stay compare-only, and CPU-selected direct or CPU-count indirect
...
- Keep the current indirect, compacted-output submission, protected
  feature-class probe, stable-rank prefix probe, projected-area probe, and
  represented-count and coverage-compensation probe paths
...
- `lod_compare_metrics.json` should include `metrics_schema_version`,
  ...
  feature-class, stable-rank prefix, projected-area, represented-count, and coverage-compensation GPU probe parity/timing
