...
prefix-selection/performance-clamped depth-windowed rank-and-feature-class filtering/compaction/checksum diagnostics for
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  metadata decoded, prefix-filtered through a performance-clamped depth-windowed
  rank/class predicate, compacted, checksummed, and converted into a diagnostic
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  decodes draw-item class/rank/depth/clamp-flag metadata, prefix-filters
  performance-clamped depth-windowed rank-limited feature
  ...
  performance-clamped depth-windowed rank-limited feature prefix dispatch over
  ...
  0, compacted up to 5,578 items at 1.85517 ms, matched previous-frame CPU/GPU
  ...
  0.578625 ms. Beauty stress recorded 1 GPU performance-clamped
  ...
  items at 2.28087 ms, matched previous-frame CPU/GPU count 2,327 with checksum
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/rank-limit/depth-window/required-flags/rejected-flags/count/checksum/ms
