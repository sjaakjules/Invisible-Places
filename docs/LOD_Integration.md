...
prefix-selection/performance-clamped projected-footprint depth-windowed rank-and-feature-class filtering/compaction/checksum diagnostics for
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  metadata decoded, prefix-filtered through a performance-clamped projected-footprint
  depth-windowed rank/class predicate, compacted, checksummed, and converted into a diagnostic
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  decodes draw-item class/rank/depth/clamp-flag plus projected-footprint/render-area
  metadata, prefix-filters performance-clamped projected-footprint depth-windowed rank-limited feature
  ...
  performance-clamped projected-footprint depth-windowed rank-limited feature prefix dispatch over
  262,115 CPU-selected draw items, checked a 131,057-item prefix with class mask
  126, rank limit 1023, depth window 2-255, required flags 4, and rejected flags
  0, projected footprint/render area window 1-1.04858e+06 px, compacted up to
  5,578 items at 2.87629 ms, matched previous-frame CPU/GPU
  count 3,886 with checksum 1,674,177,483, matched compacted indirect CPU/GPU
  vertices 3,886 / 3,886, plus 1 CPU-count GPU indirect-command dispatch at
  0.553041 ms. Beauty stress recorded 1 GPU performance-clamped projected-footprint
  depth-windowed rank-limited feature prefix dispatch over 262,132 draw items,
  checked a 131,066-item prefix with class mask 126, rank limit 1023, depth
  window 2-255, required flags 4, and rejected flags 0, compacted up to 4,365
  items at 2.31629 ms, matched previous-frame CPU/GPU count 2,327 with checksum
  1,862,844,103, and matched compacted indirect CPU/GPU vertices 2,327 / 2,327.
  Coverage ratio
  ...
  fallback or budget exceedance. The stream
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  prefix-selection/rank-and-feature-class filtering/draw-item compaction/checksum pass is
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/rank-limit/depth-window/projected-footprint-window/required-flags/rejected-flags/count/checksum/ms
