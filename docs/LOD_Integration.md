...
prefix-selection/rank-and-feature-class filtering/compaction/checksum diagnostics for
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can have representative class/rank metadata decoded,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  decodes draw-item class/rank metadata, prefix-filters rank-limited feature
  ...
  and timing are not yet available. Fast Basic recorded 1 GPU rank-limited
  feature prefix dispatch over 262,115 CPU-selected draw items, checked a
  131,057-item prefix with class mask 126 and rank limit 1023, compacted up to
  5,578 items at 4.85279 ms, and matched previous-frame CPU/GPU count 3,886
  with checksum 1,674,177,483, plus 1 GPU indirect-command dispatch at 0.576292
  ms. Beauty stress recorded 1 GPU rank-limited feature prefix dispatch over
  262,132 draw items, checked a 131,066-item prefix with class mask 126 and rank
  limit 1023, compacted up to 4,454 items at 2.674 ms, and matched
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/rank-limit/count/checksum/ms
- indirect draw count
