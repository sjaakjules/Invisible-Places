...
prefix-selection/performance-clamped represented-count-limited projected-footprint frustum-checked depth-windowed rank-and-feature-class filtering/compaction/checksum diagnostics for
diagnostic viewport draws, diagnostic compacted-count indirect command output,
and CPU-count compute-generated submitted indirect command output for eligible
viewport draws. GPU compute selection remains guarded behind feature, parity,
and timing proof; on the local MoltenVK runtime the current reported path is
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can have representative class/rank/depth/clamp-flag
  metadata decoded, source positions projected through the frame `viewProjection`,
  prefix-filtered through a performance-clamped represented-count-limited projected-footprint
  frustum-checked depth-windowed rank/class predicate, compacted, checksummed, and converted into a diagnostic
  ...
  CPU-selected compact draw-item count. Selection semantics remain unchanged.
  EXR command buffers and
...
- `lod_compare_metrics.json` schema v3 reports source/cache fingerprints,
  ...
  culling fields, GPU prefix-compaction frustum guard/position-count fields,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  build, focused GPU/LOD tests, full CTest 189/189, stream check, and
  `--lod-compare`. The local MoltenVK runtime reported GPU-selection feature
  ...
  decodes draw-item class/rank/depth/clamp-flag plus represented-count and
  projected-footprint/render-area metadata, projects source positions through the
  frame `viewProjection`, prefix-filters performance-clamped represented-count-limited
  projected-footprint frustum-checked depth-windowed rank-limited feature
  representatives inside a 1.05x guarded clip frustum, compacts, checksums CPU-selected draw items, and converts
  ...
  parity and timing are available. Fast Basic recorded 1 GPU
  frustum-checked performance-clamped represented-count-limited projected-footprint
  depth-windowed rank-limited feature prefix dispatch over 262,115 CPU-selected
  draw items with 12,183,742 positions, checked a 131,057-item prefix with class
  ...
  area window 1-1.04858e+06 px, compacted up to 5,515 items at 13.6525 ms,
  ...
  GPU indirect-command dispatch at 0.499417 ms. Beauty stress recorded 1 matching
  frustum-checked prefix dispatch over 262,132 draw items with the same 1.05x
  guard band and 12,183,742 positions, checked a 131,066-item prefix with class
  mask 126, rank limit 1023, depth window 2-255, required flags 4, and rejected
  flags 0, compacted up to 4,318 items at 8.31621 ms, matched previous-frame
  CPU/GPU count 2,301 with checksum 933,328,382, and matched compacted indirect
  CPU/GPU vertices 2,301 / 2,301.
  Coverage ratio
  stayed 1, luminance ratio 0.804235, RGB MAE 0.0152765, selection hash
  ...
  fallback or budget exceedance. The random-access frustum diagnostic was slower
  than the previous metadata-only GPU prefix compaction on the local MoltenVK/sample
  run, so it remains compare-only and the CPU fallback plus CPU-count submitted
  indirect command remain active. The stream
  check still passed at center/repeat-center 563 / 2,292 chunks,
  43,438 / 56,537 remapped draw items, 120.9 MiB CPU residency, 128.0 MiB GPU
  residency/upload, and 76.8311% chunk hit rate.
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  prefix-selection/rank-and-feature-class/frustum filtering/draw-item compaction/checksum pass is
  active for large eligible diagnostic viewport paths, but it is diagnostic
  only; GPU compute culling and authoritative selection are not active until
  parity and timing pass. Indirect draw submission and compute-generated
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/rank-limit/depth-window/projected-footprint-window/represented-source-count-window/frustum-guard/position-count/required-flags/rejected-flags/count/checksum/ms
