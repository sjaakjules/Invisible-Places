...
draw items, ordered draw-index-preserving output writes when the fitted CPU-selected range is complete, order-sensitive output identity parity, explicit submission eligibility/fallback diagnostics while CPU draw submission remains authoritative, diagnostic compacted-count indirect
...
timing per renderer profile and keeps explicit retry-window fallback reasons when the GPU path is slower. The frustum-checked shader path remains available for comparison,
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  indirect command by a compare-only compute path, while current viewport
  submitted indirect commands are still generated by compute from the
  CPU-selected compact draw-item count. A separate shader can project source
  positions through the frame `viewProjection` for frustum comparison, but that
  predicate is disabled on the default path after slower local measurements.
  Selection semantics remain unchanged. EXR command buffers and
  unsupported/small paths keep the CPU-generated or direct fallback.
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  validation, and repeat `--lod-compare` runs after the ordered full-range compaction update. The local MoltenVK
  runtime reported GPU-selection feature support plus indirect-count support; the
  runtime path now reports
  `cpu-selection+gpu-full-range-selection-compare+gpu-generated-indirect` for eligible
  viewport draws when diagnostics are enabled. The metadata compute shader decodes
  draw-item renderer profile/class/rank/depth/flags plus represented-count,
  ...
  predicate with profile mask 0, class mask 0, rank <= 2047, depth 0-255,
  represented count 0-4,294,967,295, full float area/opacity/emission windows,
  and flags +0/-0, aggregates selected items per workgroup before updating global
  compaction stats, accumulates CPU/GPU representative class counts, folds
  source-identity fingerprints from the existing source-index XOR/sum
  accumulators, checksums CPU-selected draw items, writes an ordered diagnostic
  output copy when the full fitted range is selected, and converts the compacted
  GPU count to a diagnostic indirect command. The compacted output buffer still
  has explicit submission eligibility/fallback diagnostics; CPU draw submission
  remains authoritative while count/source-fingerprint/checksum/class-count,
  ordered output-probe identity parity, and candidate-vs-reference vertex
  equivalence are measured.
  ...
  source fingerprint 2,077,166,847 / 2,077,166,847, checksum 1,323,386,965 /
  1,323,386,965, class counts spatial 2,875,473 / 2,875,473, colour 160/160,
  normal 76/76, scalar min/max/threshold 539/539 / 594/594 / 116/116,
  emissive 152/152, blue-noise 19/19, and matched compacted indirect CPU/GPU
  vertices 2,876,771 / 2,876,771. Fast Basic copied all 2,876,771 draw items
  into the diagnostic output buffer, the ordered compacted-output probe passed
  previous-frame identity 2,876,771 / 2,876,771, and the CPU/GPU output-probe
  checksum/fingerprint matched 2,354,167,959 / 2,354,167,959 and 1,664,571,717 /
  1,664,571,717. Fast Basic measured 226.173 ms for the CPU reference predicate
  and 19.3627 ms for GPU full-range compaction, so the GPU pass is measurably
  faster for the semantic-equivalent diagnostic predicate. The compacted-submission
  gate now reports eligible candidate/reference vertices 2,876,771 / 2,876,771,
  but leaves `gpu_compaction_submission_used=false` with
  ...
  Beauty stress also passed full-range count/source-fingerprint/checksum/class-count
  parity and ordered compacted-output identity, measured 22.8487 ms CPU reference
  vs 7.61833 ms GPU compaction, matched compacted indirect CPU/GPU vertices
  262,089 / 262,089, and reported compacted-submission eligible while still
  unsubmitted for the same descriptor/barrier reason. The frustum-checked shader path still exists,
  ...
  full-range compaction. Coverage ratio stayed 1, luminance ratio 0.8166, RGB MAE
  0.015853, selection hash `0xc56d6360df4e9d71`, Adaptive HQ used 1,508,358
  representatives covering 11,983,509 source points, Fast Basic submitted
  ...
  compacted-output graphics submission is wired and manually accepted; the final
  hands-on Fast Basic/Beauty/export matrix remains outstanding. The stream check still passed at center/repeat-center 563 / 2,292
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  full-range selection/rank-and-feature-class filtering/draw-item compaction/checksum pass is
  active for large eligible diagnostic viewport paths, but it is diagnostic
  only; the slower geometry-frustum predicate is disabled with an explicit
  fallback reason, and GPU compute culling and authoritative selection are not
  active until parity and timing pass. Indirect draw submission and compute-generated
  indirect commands are active for large viewport adaptive draw-item paths where
  Vulkan supports them.
...
- Keep the current indirect draw submission path guarded by runtime diagnostics.
  Move culling, projected-error evaluation, authoritative compaction/selection,
  and indirect-count work to compute only where they beat the CPU path.
...
- `lod_compare_metrics.json` should include `metrics_schema_version`,
  ...
  fields when compare-only full-range compaction is slower than the CPU reference.
