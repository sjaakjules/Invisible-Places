...
prefix-selection/compaction/checksum diagnostics for diagnostic viewport draws,
and compute-generated indirect command output for eligible viewport draws. GPU
compute selection remains guarded behind feature, parity, and timing proof; on
the local MoltenVK runtime the current reported path is
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can be prefix-filtered, compacted, and checksummed by a
  compare-only compute pass, and current viewport indirect commands can be
  generated by a compute dispatch from the CPU-selected compact draw-item count,
  so selection semantics remain unchanged. EXR command buffers and
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  prefix-filters, compacts, and checksums CPU-selected draw items and another
  ...
  and timing are not yet available. Fast Basic recorded 1 GPU prefix-selection
  dispatch over 262,115 CPU-selected draw items, selected/compacted a
  131,057-item prefix at 2.04971 ms, and matched previous-frame CPU/GPU count
  131,057 with checksum 374,011,719, plus 1 GPU indirect-command dispatch at
  0.95475 ms. Beauty stress recorded 1 GPU prefix-selection dispatch over
  262,132 draw items, selected/compacted a 131,066-item prefix at 2.28437 ms,
  and matched previous-frame CPU/GPU count 131,044 with checksum 2,926,806,065.
  Coverage ratio stayed 1, luminance ratio 0.804235, RGB MAE 0.0152765,
  selection hash `0x64f020924ab61fdd`, Adaptive HQ used 1,824,536
  representatives covering 11,848,474 source points, and the run reported no
  adaptive/full-source fallback or budget exceedance. The stream
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  prefix-selection/draw-item compaction/checksum pass is active for large
  eligible diagnostic viewport paths, but it is diagnostic only; GPU compute
  culling and authoritative selection are not active until parity and timing
  pass. Indirect draw submission and compute-generated indirect commands are
  active for large viewport adaptive draw-item paths where Vulkan supports them.
...
- GPU prefix-selection/compaction parity/status/input/selection/count/checksum/ms
- indirect draw count
