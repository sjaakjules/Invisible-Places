...
prefix-selection/depth-windowed rank-and-feature-class filtering/compaction/checksum diagnostics for
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can have representative class/rank/depth metadata decoded,
  prefix-filtered through a depth-windowed rank/class predicate, compacted, and
  checksummed by a compare-only compute pass,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  decodes draw-item class/rank/depth metadata, prefix-filters depth-windowed
  rank-limited feature
  ...
  and timing are not yet available. Fast Basic recorded 1 GPU depth-windowed
  rank-limited feature prefix dispatch over 262,115 CPU-selected draw items,
  checked a 131,057-item prefix with class mask 126, rank limit 1023, and depth
  window 2-255, compacted up to 5,578 items at 2.39829 ms, and matched
  previous-frame CPU/GPU count 3,886 with checksum 1,674,177,483, plus 1 GPU
  indirect-command dispatch at 0.623917 ms. Beauty stress recorded 1 GPU
  depth-windowed rank-limited feature prefix dispatch over 262,132 draw items,
  checked a 131,066-item prefix with class mask 126, rank limit 1023, and depth
  window 2-255, compacted up to 4,454 items at 2.25838 ms, and matched
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/rank-limit/depth-window/count/checksum/ms
- indirect draw count
