...
compaction/checksum diagnostics for diagnostic viewport draws, and
compute-generated indirect command output for eligible viewport draws. GPU
compute selection remains guarded behind
feature, parity, and timing proof; on the local MoltenVK runtime the current
reported path is `cpu-selection+gpu-compaction-compare+gpu-generated-indirect`.
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can be copied and checksummed by a compare-only compute
  pass, and current viewport indirect commands can be generated by a compute
  dispatch from the CPU-selected compact draw-item count, so selection semantics
  remain unchanged. EXR command buffers and unsupported/small paths keep the
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  `cpu-selection+gpu-compaction-compare+gpu-generated-indirect` for eligible
  viewport draws when diagnostics are enabled because a compute shader
  copies/checksums CPU-selected draw items and another compute shader writes the
  indirect command before the render pass. CPU traversal remains authoritative
  because full CPU/GPU compute-selection parity and timing are not yet
  available. Fast Basic recorded 1 GPU compaction dispatch over 2,880,612 draw
  items at 10.3514 ms with previous-frame CPU/GPU count 2,880,612 and matching
  checksum 3,163,673,222, plus 1 GPU indirect-command dispatch at 0.389083 ms, 1
  indirect draw, and 2,880,612 submitted vertices. Beauty stress recorded 1 GPU
  compaction dispatch with 262,132 max copied draw items at 32.5857 ms;
  previous-frame parity passed with CPU/GPU count 262,089 and matching checksum
  1,319,003,764, plus 1 GPU indirect-command dispatch at 0.017375 ms, 2
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  draw-item compaction/checksum pass is active for large eligible diagnostic
  viewport paths, but it is diagnostic only; GPU compute culling and
  authoritative selection are not active until parity and timing pass. Indirect
  draw submission and compute-generated indirect commands are active for large
...
- GPU compaction parity/status/count/checksum/ms
- indirect draw count
