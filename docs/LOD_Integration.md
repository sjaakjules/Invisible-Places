...
prefix-selection/feature-class filtering/compaction/checksum diagnostics for
...
- GPU-driven selection policy support exists. The renderer checks compute-queue
  ...
  viewport draw items can have representative class metadata decoded,
  prefix-filtered, compacted, and checksummed by a compare-only compute pass,
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  decodes draw-item class metadata, prefix-filters feature-class
  ...
  and timing are not yet available. Fast Basic recorded 1 GPU feature-class
  prefix dispatch over 262,115 CPU-selected draw items, checked a 131,057-item
  prefix with class mask 126, compacted up to 6,605 feature-class items at
  2.55912 ms, and matched previous-frame CPU/GPU count 4,814 with checksum
  3,666,178,065, plus 1 GPU indirect-command dispatch at 0.676 ms. Beauty
  stress recorded 1 GPU feature-class prefix dispatch over 262,132 draw items,
  checked a 131,066-item prefix with class mask 126, compacted up to 5,496
  feature-class items at 1.88237 ms, and matched previous-frame CPU/GPU count
  2,987 with checksum 2,048,260,885. Coverage ratio stayed 1, luminance ratio
  0.804235, RGB MAE 0.0152765, selection hash `0x64f020924ab61fdd`, Adaptive HQ
  used 1,824,536 representatives covering 11,848,474 source points, and the run
  reported no adaptive/full-source fallback or budget exceedance. The stream
...
- Traversal selection is still CPU-authored by default. A compare-only GPU
  prefix-selection/feature-class filtering/draw-item compaction/checksum pass is
...
- GPU prefix-selection/compaction parity/status/input/selection/class-mask/count/checksum/ms
- indirect draw count
