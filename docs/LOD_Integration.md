...
draw items, ordered draw-index-preserving output writes when the fitted CPU-selected range is complete, order-sensitive output identity parity, compare-only protected feature-class, stable-rank-prefix, hierarchy-depth, projected-area, represented-count, and coverage-compensation predicate probes, compacted-output descriptor/barrier submission after parity and performance gates pass, diagnostic compacted-count indirect
command output, full CPU-selected-range dispatch, and CPU-count compute-generated submitted indirect
command output only when compacted output is not eligible for the current image. It also tracks CPU/GPU full-range, protected feature-class, stable-rank-prefix, hierarchy-depth, projected-area, represented-count, and coverage-compensation predicate
...
- Stage 11 sample evidence on `Data/Site3-Sample-Terrestrial.ply` completed
  ...
  output identity 2,876,771 / 2,876,771. Fast Basic measured 226.799 ms for the
  CPU reference predicate and 11.3421 ms for GPU full-range compaction, so the
  ...
  checksum 3,534,349,174, and measured 8.78813 ms CPU reference vs 0.03025 ms
  ...
  checksum 3,671,843,879, and measured 58.9155 ms CPU reference vs 0.027208 ms
  ...
  4,137,926,082, and measured 22.7018 ms CPU reference vs 0.022875 ms GPU.
  ...
  checksum 65,403,358, and measured 26.8138 ms CPU reference vs 0.02275 ms
  ...
  checksum 1,624,835,333, and measured 90.9616 ms CPU reference vs 0.32425 ms
  ...
  measured 84.0577 ms CPU reference vs 0.311125 ms GPU. The
  ...
  as a first-frame fallback only, with 1 dispatch at 0.010541 ms in the latest
  sample run. Beauty stress kept max GPU point pass 0.090333 ms with EWMA
  0.0332798 ms and reported no adaptive/full-source fallback or budget
  ...
  hierarchy-depth, projected-area, represented-count, and coverage-compensation filtering stay compare-only, and CPU-selected direct or CPU-count indirect
...
- `lod_compare_metrics.json` should include `metrics_schema_version`,
  ...
  feature-class, stable-rank prefix, hierarchy-depth, projected-area, represented-count, and coverage-compensation GPU probe parity/timing
