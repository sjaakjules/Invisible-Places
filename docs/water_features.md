...
- Project schema `48`, standalone water-source schema `20`, animation schema `13`, nested Rain settings version `3`, and shared-surface binary schema `4`, with authoritative scene density groups, explicit active water-scene ownership, compact cache manifests, manual Flow spline sources, global/per-source Flow-trail visibility, connected Seepage support and live dimensions, per-node reach/width/prominence/wetting keys, Rain near-surface/ROCK/VEG tuning, independent delayed Seepage and Mesh Flow Rain response, keyed Rain/Flow/Shoreline/Mesh Flow levels, reusable water timing runs, selectable Shoreline algorithms and named profiles, v2 Ripple/Flow/Field settings, and legacy migration.
...
- Clean generated Flow branches reload from validated scene-local `.flowpathcache` sidecars when support/settings signatures still match; schema-48 project JSON stores only the compact manifest.
...
- `src/serialization/ProjectDocument.hpp` and `src/serialization/ProjectDocument.cpp`: schema-48 project state, schema-20 water-source state, schema-13 animation paths, water timing runs, explicit active water-scene ownership, manual Flow paths, global/per-source trail visibility, per-source profile locks/activity/surface guidance, automatic Mesh Flow settings/Rain envelope controls, connected Seepage node dimensions/selection limits and animation factors, selectable Shoreline styles and profile libraries, Rain settings-v3 tuning, legacy migration, and sidecar path-cache persistence.
...
The applied Shoreline wave settings remain serialized with the point-cloud style rather than in `water_ripple_layers`; that active point visual is still the render/export authority. The project profile library uses `water_shoreline_default_settings`, `water_shoreline_profiles`, and `selected_water_shoreline_profile`. Each profile snapshots both Foam Fronts and Height Foam banks, so changing algorithms after selection restores that profile's independent values.
...
- Existing embedded `water_path_cache` records are accepted as migration input when their fingerprints match, then externalized on a settled schema-48 save.
- Schema-47 projects and schema-19 water-source files have no Shoreline profile metadata. They retain the already-authored point visual unchanged; at runtime that visible Shoreline state is captured as the editable `Default` profile without being reapplied.
...
The Shoreline panel has a named profile library. `Default` is project-editable; saved custom names can be selected, reverted, or edited through a `<name>_edited` shadow before Save Profile or Discard Edits. Selecting a profile copies only its Shoreline fields into the active point visual. Viewport, still, and animation export compose the selected saved Shoreline profile into the frozen point-visual snapshot; an unsaved `_edited` shadow exports its saved base or preset. The code-owned `Calm` preset is regenerated on load, is never persisted, and is read-only until editing creates `Calm_edited`. Its Foam Fronts values are Boundary 1.595 m, Reach 2.0 m, Edge Fade 0.117 m, Direction 81 degrees, Wavelength 0.10 m, Pattern Scale 0.33, Speed 0.55, Warp 1.05, Turbulence 0.64, Density 1.0, Intensity 0.97, Emission 0.65, Opacity +0.08 / x1.25, Point Size +0 / x1.35, Colour Mix 0.75, and RGB 158/224/255.
...
- Sample scene: `tests/fixtures/sample_scene_water_sources.json` durably owns `SampleFlowPoint`, `SampleFlowPath`, and `SampleSeepage`; `scripts/generate_sample_scene_validation.py` rebuilds the schema-48 `Saved/validation/SampleSceneValidation_project.json` against the current `*. SampleScene.ply`, `Site1-Mesh-SampleScene.ply`, and `Site1-MeshSampled-5mm-SampleScene.ply` assets, with a 3 mm display, complete 2 mm terrain cache input, and no derived cache payloads.
- Serialization: schema-48 saves include Shoreline Default/custom profile metadata, authoritative scene density groups, explicit active water-scene ownership, schema-4 shared surface-cache manifests, compact Flow-path sidecar manifests, global/per-source Flow visibility, automatic Mesh Flow settings/scenario response, Rain settings-v3 controls, connected Seepage dimensions/selection limits, and persistent dormant/profile metadata for keyed water settings. Standalone water sources use schema 20 and animations use schema 13 with normalized per-node Seepage keys, Seepage Rain timing, Mesh Flow level/Rain-gain/persistence/timing, keyed Shoreline levels, and per-track timing-run assignments; the legacy per-animation `export_visuals` key is no longer written or read. Loading migrates the seeded scenarios' old display names (Pre-Colonisation Wet, Contemporary Managed) to Past/Future and Current by id, in both project files and animation tracks.
