...
- **Rain**: camera-aware GPU falling particles that slow and squish against averaged cache normals before collision and can drive three impact effects — Rings (from SAND impacts), irregular Wetness (from ROCK impacts, running further downhill the steeper the surface), and descending Droplets streams (from VEG impacts) — each shading every cloud's points inside its own height band, with Light Mist, Rain, and Heavy Downpour intensities.
...
Reach, Width, Prominence, edge feather, surface-depth tolerance, normal alignment, strength, and deterministic seed remain authored live controls. Node Strength and Prominence are deliberately decoupled: strength shapes WHERE seepage lives, prominence only scales how strongly the effect is applied. A shared area envelope (identical in the shaders and the CPU/offline evaluator) derives the run from `Reach x Strength`, lengthened up to 1.15x on near-vertical support cells and curtailed to 0.45x across flat ones, while the half-width starts at the authored Width at the node and spreads outward with travelled distance — faster on flat surfaces, slower on walls — also scaled by strength. Low strength keeps a thin band that travels mainly down steep rock; high strength spreads outward (left and right of the downhill centreline) and further. All of this reads live parameters against the per-cell normals already cached in the 16-byte support references, so keyed strength (via scenario Seepage Level and node Activity keys) reshapes the area every frame during playback and export with no topology work, while keyed Prominence independently fades the applied intensity. Selection Reach Limit and Selection Width Limit explicitly bound topology. Nodes can be selected in the viewport or list, moved back onto support, renamed, role-targeted, hidden from the viewport or export, and deleted. ROCK and VEG are enabled by default; SAND remains an explicit option. Visibility remains a parameter-only enabled factor, so off/on retains settled support and descriptors.
