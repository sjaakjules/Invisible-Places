...
The effective simulation amount combines the scenario's Mesh Flow Level, Rain Gain, and deterministic 120 Hz Rain envelope. Rise and recession times control response lag; Persistence Scale extends or contracts the settled response. Past/Future defaults to a stronger, slower-receding Rain response than Current. A stable hashed population uses `activity² × mix(0.125, 0.50, moisture)` and staggers initial births over two seconds, so parameter edits do not synchronously flash or respawn it. Scrubbing evaluates the prebuilt envelope; changing level, Rain gain, persistence, activity, dry focus, Rain-fed share/spread, speed, downhill pull, inertia, noise, wind, trail shape, or material response is parameter-ring work and does not scan PLY data, rebuild/upload the shared cache, replace descriptors, or reallocate GPU storage.
