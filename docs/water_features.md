...
- **Mesh Flow**: fixed-capacity GPU trickles over the shared Ground surface, with automatic +X-edge/concavity/Rain spawning, optional authored emitters, coherent wind, individual noise, and ROCK/VEG contact responses.
...
- Project schema `44`, standalone water-source schema `18`, animation schema `12`, nested Rain settings version `3`, and shared-surface binary schema `4`, with authoritative scene density groups, compact cache manifests, manual Flow spline sources, global/per-source Flow-trail visibility, connected Seepage support and live dimensions, per-node reach/width/prominence/wetting keys, Rain near-surface/ROCK/VEG tuning, independent delayed Seepage and Mesh Flow Rain response, keyed Rain/Flow/Mesh Flow levels, selectable Shoreline algorithms, v2 Ripple/Flow/Field settings, and legacy migration.
...
- Clean generated Flow branches reload from validated scene-local `.flowpathcache` sidecars when support/settings signatures still match; schema-44 project JSON stores only the compact manifest.
...
- Mesh Flow uses fixed-capacity GPU particle/history/contact-event resources against the resident Ground table. Automatic sources begin near the Ground bounds' +X edge, favour convergent cells when dry, and spread more widely under Rain; optional authored emitters can add art-directed sources. Individual multi-scale noise breaks up the trickles while a lower-frequency shared wind term moves the population coherently. Terrain contact fades the trail and drives bounded ROCK/VEG colour, opacity, emission, and twinkle responses.
...
- `dynamicMeshFlowSettings`, authored emitters, the current fixed GPU session, reset/parameter revisions, and diagnostics own Mesh Flow's small authored/runtime state. Particle positions, history, contact events, and the resident Ground table remain renderer-owned transient resources rather than a generated point cloud.
...
For grouped LiDAR scenes, the selected three-role display bundle loads and becomes visible first. Canonical analysis roles load on demand for explicit Bake Path and analysis-based Ripple/Field work; Seepage and Mesh Flow emitter placement use visible depth plus the resident shared cache. The renderer uploads and draws only committed display roles, and staged switch targets are not renderable. This canonical analysis set is distinct from the shared surface-cache input: after the visible display upload completes, the cache streams each file in the exact complete 2 mm bundle once (or one nearest complete fallback bundle) plus the active scene's 5 mm `MESHSampled` Ground source, without loading any of them as display or analysis data.
...
- Existing embedded `water_path_cache` records are accepted as migration input when their fingerprints match, then externalized on a settled schema-44 save.
...
- Mesh Flow Ground sparse XY cells retain the highest occupied sampled-Ground 10 mm cell, averaged normal/downhill direction, convergence/confidence, component identity, and eight-neighbour connectivity. Samples below terrain tolerance are discarded; contact-level samples terminate a trickle, and upper vegetation-supported components remain eligible flow surfaces. Ground stays a distinct role and is never treated as Rain collision or Seepage terrain.
...
The default allocation contains 4,096 particles and 24 history samples per particle. Allocation-changing capacity/history edits request a fenced reset; normal activity and look edits retain the allocation. Each simulation step performs bounded Ground hash queries, follows the cached downhill vector with inertia, adds deterministic per-particle noise at a configurable scale/speed, then adds a lower-frequency shared wind term. Individual noise prevents uniformly parallel paths while the shared term gives the population the coherent movement expected from weather under vegetation.

With **Automatic Sources** enabled, dry spawning favours convergent Ground cells in a band near the surface bounds' +X edge. Rain widens the candidate distribution according to Rain Spawn Spread so water can trickle across the understory rather than only at dry seepage-prone depressions. Authored emitters placed through visible-scene depth plus the resident Ground cache can add or replace that distribution; they remain active-scene project state and do not enter ordinary Flow's `WaterPathCache`.

The effective simulation amount combines the scenario's Mesh Flow Level, Rain Gain, and its deterministic 120 Hz Rain envelope. Rise and recession times control response lag; Persistence Scale extends or contracts the settled response. Pre-Colonisation Wet defaults to a stronger, slower-receding Rain response than Contemporary Managed. Scrubbing evaluates the prebuilt envelope, so changing level, Rain gain, persistence, activity, speed, downhill pull, inertia, source band, dry focus, Rain spread, noise, wind, trail shape, or material response is parameter-only and does not rebuild/upload the shared cache.

Trails are surface-offset elongated GPU samples. A Ground cell marked as terrain contact ends the upper-surface route, fades the trail over Contact Fade, and writes a bounded contact event. ROCK response controls radius, opacity, emission, colourisation, and persistence; VEG adds its own radius/appearance, twinkle, and downward stream depth. These contact effects visually modify the active ROCK/VEG draw without converting Ground into an authored terrain role. `Show Trails` is a presentation gate and retains the settled simulation.
...
- Sample scene: `tests/fixtures/sample_scene_water_sources.json` durably owns `SampleFlowPoint`, `SampleFlowPath`, and `SampleSeepage`; `scripts/generate_sample_scene_validation.py` rebuilds the schema-44 `Saved/validation/SampleSceneValidation_project.json` with a 3 mm display, complete 2 mm terrain cache input, the 5 mm SampleScene `MESHSampled` Ground source, and no derived cache payloads.
- Serialization: schema-44 saves include authoritative scene density groups, schema-4 shared surface-cache manifests, compact Flow-path sidecar manifests, global/per-source Flow visibility, Mesh Flow settings/scenario response, Rain settings-v3 controls, and connected Seepage dimensions/selection limits. Standalone water sources use schema 18 and animations use schema 12 with normalized per-node Seepage keys, Seepage Rain timing, and Mesh Flow level/Rain-gain/persistence/timing.
...
- Mesh Flow: automatic +X-edge dry/concavity and Rain-spread spawning, authored emitters, fixed-capacity particle/history resources, individual noise/shared wind, and terrain-contact events all use the resident Ground view. Live scenario/style edits advance parameter/dispatch counters without rebuilding the cache, warming a triangle cache, or creating a CPU point cloud; capacity/history edits perform a fenced allocation replacement.
...
- Stream schema: generated ordinary Flow/Field and GPU Mesh Flow Trail scalar fields match the exact order above; Mesh Flow additionally retains a dedicated fixed particle/history/contact-event GPU ABI without a generated CPU cloud.
