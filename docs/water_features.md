...
- Project schema `42`, standalone water-source schema `17`, animation schema `10`, and shared-surface binary schema `3`, with authoritative scene density groups, compact cache manifests, manual Flow spline sources, per-source Flow profile locks/activity/surface guidance, four Seepage patterns, per-node wetting keys, delayed Seepage Rain response, keyed Rain and Flow levels, selectable Shoreline algorithms, v2 Ripple/Flow/Field settings, GPU Rain settings, and legacy migration.
...
- Clean generated Flow branches reload from validated scene-local `.flowpathcache` sidecars when support/settings signatures still match; schema-42 project JSON stores only the compact manifest.
...
- Rain uses 32,768 persistent GPU particle slots and a dedicated depth-tested streak draw. One shared, persisted 20 mm `WaterSurfaceCache` stores ROCK/SAND top surfaces and VEG 3D occupancy for Rain plus orientation-independent ROCK/SAND 3D surfels used by Flow and Seepage guide tracing. It warms after the active scene's visible display upload whether the effects are enabled or not.
...
- `src/serialization/ProjectDocument.hpp` and `src/serialization/ProjectDocument.cpp`: schema-42 project state, schema-17 water-source state, schema-10 animation paths, manual Flow paths, per-source profile locks/activity/surface guidance, Seepage nodes/looks/node tracks/Rain timing, selectable Shoreline styles, v2 water settings/layers, GPU Rain settings/visuals, legacy migration, and sidecar path-cache persistence.
...
For grouped LiDAR scenes, the selected three-role display bundle loads and becomes visible first. Canonical analysis roles load on demand for explicit Bake Path, analysis-based Ripple/Field work, and Seepage placement/movement; they are not a startup-residency requirement. The renderer uploads and draws only committed display roles, and staged switch targets are not renderable. This canonical analysis set is distinct from the shared surface-cache input: after the visible display upload completes, the cache streams exact 5 mm roles once and does not routinely load the much larger analysis clouds.

A display switch performs no synchronous Ripple, Field, Flow, or Seepage topology rebuild. Ripple may restore an exact-source membership cache after commit; otherwise its state remains dirty until explicit recalculation. Field presentation is likewise display-source-specific and never reuses analysis point indices across variants. Seepage retains its world-space nodes, display-independent eight-station guides, and compact topology, while role filtering and Auto quality use the current render layer. Generated streams, Rain, and the shared surface cache remain independent of display density.
...
Project documents now use schema `42`. The authoritative grouped-density record and scene cache manifest are:
...
- Existing embedded `water_path_cache` records are accepted as migration input when their fingerprints match, then externalized on a settled schema-42 save.
...
Binary cache schema 3 (`IPWSC003`) persists the scene-relative source signature, aggregate arrays, all three GPU-ready power-of-two hash tables, and a streaming payload checksum. Its identity trailer can be verified during the same sequential read, so a warm launch does not rescan PLY payloads, reconstruct tables, or perform a second whole-payload hash. The signature covers source-relative files, roles, spacings, transforms, file sizes/timestamps, algorithm schema, and 20 mm resolution while excluding all Rain, Flow, Seepage, visibility, animation, and visual parameters. New caches live at `<scene>/.invisible_places/cache/water/<signature>.surfacecache`, with project-local hidden fallback when scene storage is unavailable. Before a cold PLY scan, the loader also probes the fallback and legacy schema-2 (`IPWSC002`) `.raincache` locations, then migrates a valid payload to the neutral sidecar. Payloads above 5 GiB remain runtime-only and do not retain a misleading manifest path.

The active grouped scene starts warming only after its selected display bundle is uploaded, even when Rain, Flow, and Seepage are disabled. Point-cloud loading, surface build/load plus GPU preprocessing, and dynamic-mesh cache warmup share one exclusive high-memory slot; the surface cache takes priority after display commit and releases the slot only when preprocessing completes. The last settled cache remains resident until a replacement or active-scene change. `UploadWaterSurfaceCache` ignores a repeated ready or pending revision, uploads the immutable tables once, and submits one edge-preserving 3D Flow-normal preprocessing dispatch. `WaterSurfaceFlowView()` exposes the filtered table directly to GPU Flow route generation. Deterministic CPU Flow fallback and Seepage guide construction use the persisted aggregate; Seepage does not pass the global table to its point shaders.
...
The neutral runtime API is `WaterSurfaceCache`/`WaterSurfaceSource`, `Build`/`Save`/`UploadWaterSurfaceCache`, `WaterSurfaceFlowView()`, and `WaterSurfaceUploadRevision()`; old Rain-cache runtime aliases are not the active contract. Each schema-42 scene manifest records path/size/checksum/source fingerprint and requested/built rebuild generations; runtime derives Missing/Stale/Building/Valid/Failed status from it. The panel exposes those values and **Rebuild Cache**. Rebuild increments the requested generation and keeps the settled GPU resource active until the replacement finishes; Valid is published only after preprocessing, and Failed waits for another explicit request instead of looping every frame.
...
SAND impacts produce short expanding rings at the real collision position using the Rain Rings kernel. ROCK impacts use two-thirds of the former planar area and twice the growth duration. Their centre drifts by at most 20% of that reduced radius along gravity projected onto the impact surface; lower-Z points receive a slightly stronger response and begin fading later, so dampness settles gently downhill while the total event lifetime remains unchanged. VEG impacts affect a narrow XY column below the collision; vertical-distance delay, seeded variation, and independent fading create a downward sequential twinkle. These world-space effects apply consistently to 3 mm preview and 1 mm output without gathering affected point indices on the CPU.
...
- Sample scene: `tests/fixtures/sample_scene_water_sources.json` durably owns `SampleFlowPoint`, `SampleFlowPath`, and `SampleSeepage`; `scripts/generate_sample_scene_validation.py` rebuilds the schema-42 `Saved/validation/SampleSceneValidation_project.json` with a 3 mm display, exact 5 mm cache sources, and no derived cache payloads.
- Serialization: schema-42 saves include authoritative scene density groups, shared surface-cache manifests, and compact Flow-path sidecar manifests plus manual Flow path, per-source Flow profile locks/activity/surface guidance, four-pattern Seepage/Ripple/Flow/Mesh Flow/Field/Rain state, keep both SAND Shoreline tuning banks in point-cloud style data, and omit Basin/Runoff/Caustic region keys. Standalone water sources use schema 17 and animations use schema 10 with normalized per-node Seepage tracks and Rain timing.
...
- Density switch: 1/2/3/5 mm display changes leave analysis support signatures, Seepage topology/guides, settled Flow/Field output, and shared-surface revisions unchanged. An exact-source Ripple cache may restore after commit; otherwise Ripple/Field stays dirty until explicit recalculation, with no synchronous switch-time topology work.
- Legacy load: old Caustic regions become Ripple `Caustic Lace`; old Basin/Runoff records are ignored.
- Shared cache: one exact-5-mm source scan produces the Rain and Flow/Seepage tiers; a schema-3 `.surfacecache` warm load performs no PLY scan or CPU hash build, while schema-2 `.raincache` is migration-only. Repeated polling uploads one revision, explicit rebuild advances requested/built generations, and display/effect edits leave these counters unchanged.
...
- Motion: one immutable `WaterFrameState` supplies raw Rain/Flow and delayed Seepage state per live/export sample. Keyed Flow uses a deterministic Trail subset without geometry uploads; normalized Seepage node keys and its delayed envelope retain guide/hash topology; Rain remains immediate.
