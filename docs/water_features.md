...
- **Mesh Flow**: automatic fixed-capacity GPU trickles over the active scene's shared Ground surface, with vegetation-supported highest-+X dry emergence, VEG Rain-fed distributed emergence, coherent wind, individual noise, and ROCK/VEG contact responses.
...
- Clean generated Flow branches reload from validated scene-local `.flowpathcache` sidecars when support/settings signatures still match; schema-45 project JSON stores only the compact manifest.
...
Project documents now use schema `45`. Standalone water-source documents use schema `19`, animation paths use schema `12`, nested Rain settings use version `3`, and binary surface caches use schema `4`. Schema 45 records the active water scene group explicitly so scene-local water state is not selected by array order. The authoritative grouped-density record and scene cache manifest are:
...
- Existing embedded `water_path_cache` records are accepted as migration input when their fingerprints match, then externalized on a settled schema-45 save.
...
- Mesh Flow Ground sparse XY cells retain the highest occupied sampled-Ground 10 mm cell, averaged normal/downhill direction, convergence/confidence, component identity, and eight-neighbour connectivity. Samples below terrain tolerance are discarded; contact-level samples terminate a trickle, and upper vegetation-supported components remain eligible flow surfaces. One immutable upload-time table retains eligible vegetation-supported cells near each component's highest-+X edge. Ground stays a distinct role and is never treated as Rain collision or Seepage terrain.
...
Mesh Flow is always automatic. Ground-v3 preprocessing builds a compact entry table from vegetation-supported cells within `max(0.75 m, 4% of component X extent)` of each connected component's highest-+X boundary. Dry spawning strongly favours convergent/rill cells from that table. There is no authored Mesh-source list: ordinary Flow points, paths, emitters, motions, and attractors do not enter Mesh Flow or produce seeds. Legacy Mesh-source fields remain readable for migration but are inactive and omitted on the next settled save.

Rain simulation writes VEG collisions into a fixed 1,024-entry GPU-only ring. Mesh Flow consumes the preceding completed submission without CPU readback or event scanning; the existing leaf-to-leaf VEG response remains first, and a Rain-fed particle can emerge only after that propagation could reach the highest connected vegetation-supported Ground cell below the hit. At maximum moisture, distributed Rain sources can supply up to the authored Rain-fed share while the rest of the stable population continues to use the highest-+X entry table. Rain-fed particles remember their Ground source during the recession envelope.

The effective simulation amount combines the scenario's Mesh Flow Level, Rain Gain, and deterministic 120 Hz Rain envelope. Rise and recession times control response lag; Persistence Scale extends or contracts the settled response. Pre-Colonisation Wet defaults to a stronger, slower-receding Rain response than Contemporary Managed. A stable hashed population uses `activity² × mix(0.125, 0.50, moisture)` and staggers initial births over two seconds, so parameter edits do not synchronously flash or respawn it. Scrubbing evaluates the prebuilt envelope; changing level, Rain gain, persistence, activity, dry focus, Rain-fed share/spread, speed, downhill pull, inertia, noise, wind, trail shape, or material response is parameter-ring work and does not scan PLY data, rebuild/upload the shared cache, replace descriptors, or reallocate GPU storage.
...
- Sample scene: `tests/fixtures/sample_scene_water_sources.json` durably owns `SampleFlowPoint`, `SampleFlowPath`, and `SampleSeepage`; `scripts/generate_sample_scene_validation.py` rebuilds the schema-45 `Saved/validation/SampleSceneValidation_project.json` against the current `*. SampleScene.ply`, `Site1-Mesh-SampleScene.ply`, and `Site1-MeshSampled-5mm-SampleScene.ply` assets, with a 3 mm display, complete 2 mm terrain cache input, and no derived cache payloads.
- Serialization: schema-45 saves include authoritative scene density groups, explicit active water-scene ownership, schema-4 shared surface-cache manifests, compact Flow-path sidecar manifests, global/per-source Flow visibility, automatic Mesh Flow settings/scenario response, Rain settings-v3 controls, and connected Seepage dimensions/selection limits. Standalone water sources use schema 19 and animations use schema 12 with normalized per-node Seepage keys, Seepage Rain timing, and Mesh Flow level/Rain-gain/persistence/timing.
...
- Mesh Flow: per-component vegetation-supported highest-+X dry emergence, the 1,024-entry VEG Rain seed ring, fixed 4,096-particle/24-history resources, individual noise/shared wind, and terrain-contact events all use the explicitly owned resident Ground-v3 view. Ordinary Flow sources produce zero Mesh Flow seeds. Live scenario/style edits advance parameter/dispatch counters without rebuilding the cache, warming a triangle cache, creating a CPU point cloud, replacing descriptors, or reallocating fixed GPU storage.
