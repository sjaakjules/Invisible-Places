...
Vegetation Interception, Surface Runoff, and Splash are independent route options. With vegetation interception enabled, drops that cross VEG support points catch there and add a short drip motion; if Surface Runoff is enabled, the drip continues to the mesh and then sheds downhill, otherwise it fades at the vegetation. Smooth mesh floor under vegetation gets deterministic surface roughness so runoff trickles instead of moving in a straight line, while nearby ROCK support suppresses that roughness so high-detail rock areas use the mesh/rock surface detail directly. With runoff enabled, mesh/support hits continue over downhill surface anchors toward SAND; with runoff disabled, drops stop and fade at the first mesh/support impact. Splash defaults off; when enabled, non-SAND/non-VEG mesh impacts emit a few short secondary rain routes, capped by `splashMaxDistanceMeters`, scaled by fall speed, rain width, intensity preset, and `splashStrength`. When a runoff route reaches SAND it may continue for `sandRunDistanceMeters` and then terminate; without SAND it terminates on fallback support, and without a support hit it falls to a no-hit kill plane below the scene. Route anchors store time fractions, so falling stays fast while surface runoff/shedding is slower. Diagnostics report requested/emitted drops, visible samples, route anchors, first support hits, mesh hits, vegetation drips, splash droplets, impact stops, sand terminations, fallback terminations, and no-support kills.
...
Water > Rain controls include enablement, intensity preset, Vegetation Interception, Surface Runoff, Splash, drop count, fall speed, surface run speed, sand run distance, splash strength/reach/droplet limit, wind direction/strength/noise/response, spawn height/radius, camera death distance, route anchor count, seed, support sample count, and Regenerate Rain. Rain Visual uses the shared trail profile editor for trail length, width, point spacing, streak length, colour, opacity, and emission. Rain intensity presets scale density, speed, width/streak length, wind response/noise, splash potential, and visual wetness without changing Flow or Field semantics.
