...
- `Contour Pulses` sends repeating intensity fronts down the same least-resistance cost metric used by live Node Strength. The centre of each front leads its side edges, stable 3D noise wrinkles the boundary, and a quieter second train travels at a slightly different speed so fronts split and blend without becoming uniform horizontal bands. Pulse Spacing, Pulse Width, Pulse Speed, Front Irregularity, Evolution, and the shared reflection controls are parameter-only; Strength expansion, Rain, scrubbing, and export never rebuild connected support.
