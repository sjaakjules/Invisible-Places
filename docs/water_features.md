...
- Clean generated Flow branches reload from validated scene-local `.flowpathcache` sidecars when support/settings signatures still match; schema-47 project JSON stores only the compact manifest.
...
- Existing embedded `water_path_cache` records are accepted as migration input when their fingerprints match, then externalized on a settled schema-47 save.
...
- `Contour Pulses` circulates multiple independently launched intensity waves over the accumulated downhill run. The reference wave launches at the source at animation time zero so selecting the pattern gives immediate feedback; every other wave has a stable random start phase, and all waves retain independent width, amplitude, and speed around Pulse Speed, so faster waves overtake slower ones instead of moving one periodic overlay. A stationary 3D surface field wrinkles the broad feathered fronts without sliding down the rock. Isolated waves are deliberately subtle; their additive catch-up overlap ramps quickly into irregular high-intensity regions. Pulse fronts carry transient wetness even when Base Wetness is zero; Base Wetness controls the persistent moisture left between fronts. The authored waves are combined once per node and frame into a stable maximum-span one-dimensional field with 32/64/128 samples for Low/Balanced/High quality. Each rendered point performs one interpolated lookup plus the quality-scaled stationary noise instead of looping over every wave. Fractional Wave Count fades the next wave in continuously, and live Strength/Rain only reveals the pre-phased field, so waves do not teleport when extent changes. The CPU/offline evaluator builds and samples the same field. Pulse Spacing, Pulse Width, Pulse Speed, Wave Count, Speed Variation, Front Irregularity, Evolution, and the shared reflection controls remain parameter-only.
...
- Sample scene: `tests/fixtures/sample_scene_water_sources.json` durably owns `SampleFlowPoint`, `SampleFlowPath`, and `SampleSeepage`; `scripts/generate_sample_scene_validation.py` rebuilds the schema-47 `Saved/validation/SampleSceneValidation_project.json` against the current `*. SampleScene.ply`, `Site1-Mesh-SampleScene.ply`, and `Site1-MeshSampled-5mm-SampleScene.ply` assets, with a 3 mm display, complete 2 mm terrain cache input, and no derived cache payloads.
- Serialization: schema-47 saves include authoritative scene density groups, explicit active water-scene ownership, schema-4 shared surface-cache manifests, compact Flow-path sidecar manifests, global/per-source Flow visibility, automatic Mesh Flow settings/scenario response, Rain settings-v3 controls, connected Seepage dimensions/selection limits, and persistent dormant/profile metadata for keyed water settings. Standalone water sources use schema 19 and animations use schema 13 with normalized per-node Seepage keys, Seepage Rain timing, Mesh Flow level/Rain-gain/persistence/timing, keyed Shoreline levels, and per-track timing-run assignments; the legacy per-animation `export_visuals` key is no longer written or read. Loading migrates the seeded scenarios' old display names (Pre-Colonisation Wet, Contemporary Managed) to Past/Future and Current by id, in both project files and animation tracks.
