...
- Project schema `38` with authoritative scene density groups, manual Flow spline sources, Seepage nodes/shared looks, water scenarios, selectable Shoreline algorithms, v2 Ripple/Flow/Field settings, GPU Rain settings, saved Flow caches, and legacy Caustics-to-Ripples migration.
...
- Rain uses 32,768 persistent GPU particle slots and a dedicated depth-tested streak draw. A static 20 mm cache stores ROCK/SAND top surfaces and VEG 3D occupancy, so wind-slanted drops stop at their earliest role hit without route generation or surface runoff.
...
- `src/app/Application.cpp`: Water panel UI, runtime `WaterWorkflowState`, emitter/node/region editing, Seepage GPU refresh orchestration, Flow/Field cache orchestration, asynchronous Rain collision-cache lifecycle, and project/source document wiring.
- `src/serialization/ProjectDocument.hpp` and `src/serialization/ProjectDocument.cpp`: schema-38 project state, schema-14 water-source state, manual Flow paths, Seepage nodes/looks, selectable Shoreline styles, v2 water settings/layers, GPU Rain settings/visuals, legacy migration, and path cache persistence.
- `src/water/RainSimulation.*`, `src/renderer/pointcloud/PointCloudPreviewState.*`, `src/renderer/core/VulkanViewportShell.cpp`, and root `shaders/rain_*` / `shaders/pointcloud_*`: streamed Rain cache construction, deterministic CPU parity, persistent GPU simulation, role-indexed impacts, water overlays, SAND Shoreline, compact Seepage, and stream surfel shading.
...
- `collisionRainSettings`, `rainVisual`, one shared `rainCollisionCache`, asynchronous warmup state, and cache/upload diagnostics for the dedicated Rain runtime.
- `activeRegionFeature`, `regionEditor`, and placement flags for editable Ripple regions and legacy-safe region editing.

Generated water overlay sessions are excluded from support-layer discovery and from base-cloud look-dev/export visual selection. They are renderable water output, not source LiDAR layers for future water bakes. Ripples no longer create active visible `-Ripples.generated` sessions; their display-source-specific membership and procedural params are uploaded to the committed base-cloud renderer instead. Shoreline waves are not generated sessions either; they are evaluated during the committed SAND point-cloud draw. Seepage likewise remains on the base draw and uploads only compact nodes, occupied hash cells, and bounded node references. Field Surface Motion contributes to display-cloud composition through spatially remapped `water_effect_*` fields, while Flow trails, Mesh Flow, and Field Streamlines remain generated overlay sessions. Rain is rendered by its dedicated shared GPU resources.
...
Project documents now use schema `38`. The authoritative grouped-density record remains:
...
`water_sources.json` schema `14` mirrors the active source/layer/settings subset for reusable water setup, including manual Flow paths, Seepage nodes/looks, the same Ripple/Flow/Field/Rain settings, and the current Flow path cache when available. Schema-11 and older files load with an empty manual-path list.
...
Lane and Trail regeneration is derived, cancelable background work. Slider edits debounce for 150 ms and release the latest request immediately; stale requests stop between paths, trails, and scalar-packing blocks. Per-source fingerprints reuse unchanged point/manual trail artifacts, so hiding a Path View branch rebuilds only its owning emitter. The selected branch and its existing trail samples are masked immediately, then the requested trail count is redistributed across the remaining branches in the background. Prepared cumulative arc-length tables replace repeated full-path scans during route sampling, while speed-only edits update only affected sources and the contiguous `trail_speed` GPU scalar block. The Flow panel reports build, scalar-pack, upload, source reuse, sample, and cancellation diagnostics.
...
When a grouped scene becomes active, Rain selects the exact 5 mm ROCK, SAND, and VEG files. A missing role uses its coarsest available source and emits one warning. A position/normal-only streaming PLY reader reduces those sources into a fixed 20 mm cache without constructing full `LoadedPointCloud` objects or retaining colour/scalar data. The cache is persisted under `Saved/cache/rain/` with an algorithm, source metadata, transform, and resolution signature; a valid file is loaded and uploaded directly on later runs.

ROCK and SAND occupy a sparse XY hash with separate top heights, averaged normals, and confidence. VEG occupies a sparse 3D hash so a slanted drop can strike foliage at different elevations. Both GPU tables use power-of-two open addressing, no more than 65 percent occupancy, and bounded probes. Particle movement traverses 20 mm cells with DDA and chooses the earliest VEG, ROCK, or SAND segment hit. A hit always stops and deterministically respawns the drop; disabling effects never disables collision.
...
Native GPU validation is available through `--gui-smoke rain-gpu-sample-scene` and `--gui-smoke rain-gpu-scene1-3mm`. Both scenarios verify fixed buffer capacities, effects-off dispatch skipping, uniform-only live edits, stable collision-upload revisions, and Light Mist/Rain/Heavy Downpour EXR comparisons with reactions on and off. The Scene1 scenario uses the exact 5 mm collision sources while displaying the complete 3 mm role bundle.

## Stream Surfel Scalar Contract

Generated Flow trails, Mesh Flow trails, and Field Streamlines must expose these scalar fields in this order:
...
Generated Flow, Mesh Flow, and Field stream layers participate in viewport rendering and the same EXR/MP4 export path as other visible point-cloud sessions. Dedicated Rain compute, streak drawing, and role impacts are recorded in those same Vulkan frame/export command buffers. The software offline renderer accepts one deterministic `RainSimulator` particle/event frame shared across its tiles. Water streams and effect layers are not exported as PLY unless a future explicit export feature asks for it. Ripples evaluate through sparse base-cloud runtime memberships/params in viewport and offline export. Seepage evaluates compact spatial-grid buffers in the Vulkan point paths. Field Surface Motion evaluates through active-cloud `water_effect_*` composition.
...
The Visuals tab's scene-wide **Visible Point Cloud** selector changes only presentation density. The old ROCK/SAND/VEG bundle stays visible until all target roles and their Ripple/Field payloads are ready, then commits atomically. Seepage nodes remain world-space authored state and their compact grids are rebuilt for the committed role layers without copying point indices. Water support caches, generated stream overlays, and the Rain collision-cache build/upload revisions remain untouched. Viewport and offline/export paths exclude CPU-only analysis sources and staged targets.
...
- Serialization: schema-38 saves include authoritative scene density groups plus manual Flow path, Seepage/Ripple/Flow/Mesh Flow/Field/Rain state, keep both SAND Shoreline tuning banks in point-cloud style data, and omit Basin/Runoff/Caustic region keys.
...
- Density switch: 1/2/3/5 mm display changes leave analysis support signatures, path/field/trail caches, and Rain collision-cache revisions unchanged; Ripple memberships stay within the exact committed display cloud, Seepage rebuilds compact per-role grids from world-space nodes, and Field presentation data is spatially remapped.
...
- Motion: Flow and Field Streams animate through route/scalar playback; Rain advances and respawns persistent GPU particles through compute with no generated route cloud.
...
- Export: visible generated Flow/Mesh Flow/Field stream layers, dedicated GPU Rain and role impacts, sparse Ripple runtime effects, SAND Shoreline waves, compact-grid Seepage, and active-cloud Field Surface `water_effect_*` fields appear in viewport and camera export paths without requiring water PLY export.
