...
- `Contour Pulses` circulates multiple independently launched intensity waves over the same least-resistance cost metric used by live Node Strength. Each wave has a stable random start phase, width, amplitude, and speed around Pulse Speed, so faster waves overtake slower ones instead of moving one periodic overlay. A stationary 3D surface field wrinkles the broad feathered fronts without sliding down the rock. Isolated waves are deliberately subtle; their additive catch-up overlap ramps quickly into irregular high-intensity regions. Pulse Spacing, Pulse Width, Pulse Speed, Wave Count, Speed Variation, Front Irregularity, Evolution, and the shared reflection controls are parameter-only; Strength expansion, Rain, scrubbing, and export never rebuild connected support.
