...
Vegetation Interception and Surface Runoff are independent route options. With vegetation interception enabled, drops that cross VEG support points catch there, add a short drip motion, and fade instead of passing through to the mesh. With runoff enabled, mesh/support hits continue over downhill surface anchors toward SAND; with runoff disabled, drops stop and fade at the first mesh/support impact. When a runoff route reaches SAND it may continue for `sandRunDistanceMeters` and then terminate; without SAND it terminates on fallback support, and without a support hit it falls to a no-hit kill plane below the scene. Route anchors store time fractions, so falling stays fast while surface runoff/shedding is slower. Diagnostics report requested/emitted drops, visible samples, route anchors, first support hits, mesh hits, vegetation drips, impact stops, sand terminations, fallback terminations, and no-support kills.
...
Water > Rain controls include enablement, intensity preset, Vegetation Interception, Surface Runoff, drop count, fall speed, surface run speed, sand run distance, wind direction/strength/noise/response, spawn height/radius, camera death distance, route anchor count, seed, support sample count, and Regenerate Rain. Rain Visual uses the shared trail profile editor for trail length, width, point spacing, streak length, colour, opacity, and emission. Rain intensity presets scale density, speed, width/streak length, wind response/noise, and visual wetness without changing Flow or Field semantics.
