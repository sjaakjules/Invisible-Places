...
The Visuals tab's scene-wide **Visible Point Cloud** selector changes only presentation density. The old ROCK/SAND/VEG bundle stays visible until all target roles are ready, then commits atomically. Seepage nodes remain world-space authored state and existing compact topology is reused; Flow output and the shared surface cache are likewise untouched. Viewport and offline/export paths exclude CPU-only analysis sources and staged targets.
