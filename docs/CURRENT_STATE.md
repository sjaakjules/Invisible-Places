...
- Scenario and look animation changes compact parameter buffers only. Each live or export sample resolves one immutable `WaterFrameState` containing normalized/sample time, the raw scenario, the Seepage-delayed scenario, the independently filtered Mesh Flow moisture value, and per-node states. Rain and ordinary Flow consume the raw state; Seepage uses its deterministic 120 Hz Delay/Rise/Recession moisture envelope, while Mesh Flow uses its own deterministic 120 Hz Rain rise/recession/persistence envelope. Visible Rain is not delayed and out-of-order scrubbing remains reproducible. Keyed Flow reveals a deterministic subset of prebuilt trails; keyed Mesh Flow changes active fixed-capacity particles and appearance without reallocating its Ground table, history, or contact-event buffers. A long Rain/Flow/Seepage/Mesh Flow animation therefore does not rebuild connected Seepage support, rebuild Flow geometry, warm a triangle cache, dispatch CPU route generation, or upload point-cloud/shared-cache data.
...
- Project serialization uses schema version 44 for the current project shape; standalone water-source documents use schema version 18. Animation-path saves use schema version 12, nested Rain settings remain version 3, and the shared binary water-surface cache uses schema version 4.
...
- Project documents also persist Water v2 authored state plus compact shared-surface and Flow-path manifests. Clean generated Flow branches and branch bake fingerprints live in `.flowpathcache`; generated cache arrays are not embedded in schema-44 JSON. Mesh Flow persists fixed-capacity simulation/style settings and global scenario values for level, Rain gain, persistence scale, and Rain rise/recession; its particle/history/contact state remains transient GPU data.
...
- `Generated SampleScene validation project round-trips schema 44 without derived caches`
- `Schema 44 externalizes clean Flow path caches and prunes orphaned data`
