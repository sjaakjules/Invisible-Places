...
- Shader shoreline waves are role-gated to SAND in grouped scenes. `Foam Fronts (Current)` preserves the existing `SandCloudShorelineWaveValue` path; `Height Foam` adds independent run-up/break heights, persistent offshore foam, stronger incoming gathers, and fading returns. The Shoreline panel provides an editable Default, saved named profiles, and the code-owned read-only Calm preset; selection copies both algorithm banks into the active point visual without replacing unrelated visual settings. Neither algorithm allocates region memberships or dense scalar fields.
...
- Animation schema 13 stores normalized global water scenario keys with complete look snapshots, Seepage level/spread, continuous Rain and Flow levels, Mesh Flow level/Rain gain/persistence/rise/recession, Seepage Rain delay/rise/recession, outgoing Smooth/Linear/Hold interpolation, and embedded linked-scenario fallbacks. Each track can also store normalized per-node Activity, Local Spread, Wetting Front, Reach Scale, Width Scale, and Prominence keys by stable node ID. Animation-v10 `localSpread` remains readable; missing new factors default to one. The UI displays and edits key time in seconds while storage remains normalized. Water keys retain their timing when animation duration, frame rate, or camera keys change; copying water keying does not copy those animation properties.
...
- Project serialization uses schema version 48 for the current project shape; standalone water-source documents use schema version 20. Animation-path saves use schema version 13, nested Rain settings remain version 3, and the shared binary water-surface cache uses schema version 4.
...
- Animation paths preserve water caustic look settings for current visual style compatibility and schema-13 normalized Seepage/Rain/Flow/Mesh Flow scenario tracks, nested per-node Seepage keys, and embedded scenario fallbacks for synchronized playback and export. Schema-10 Local Spread keys migrate with Reach Scale, Width Scale, and Prominence defaulting to one; missing Seepage Rain timing remains immediate. Older documents default Mesh Flow to level 1, Rain gain 0, persistence scale 1, and immediate Rain timing. They also default to full ordinary Flow (`flowLevel = 1`), visible global/source trails, source strength 1, Rain response 0, and disabled Surface Guide on legacy manual paths; newly authored manual paths enable Surface Guide. Existing saved looks without a pattern, and unknown or removed pattern names such as `legacy_ripples`, load as Chaotic Bloom.
