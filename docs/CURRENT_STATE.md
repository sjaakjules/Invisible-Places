...
- Scenario and look animation changes compact parameter buffers only. Spatial bounds are pre-indexed for maximum scenario-plus-Rain expansion. Continuous keyed Rain reveals and scales a deterministic fraction of a maximum fixture built once for playback, so scrubbing does not regenerate rain routes.
...
- Grouped ROCK/SAND/VEG scenes build combined water support only from CPU-ready canonical analysis sources, not from the selected display bundle. Display switching therefore leaves support signatures, path/field/trail caches, source placement, flow/rain collision, and region analysis unchanged.
...
- Flow trails, Mesh Flow trails, Field Streamlines, and Rain trails share the animated `stream_*` scalar schema. Flow follows path anchors through Lane profiles, Mesh Flow follows triangle-mesh routes with `feature_type = 5`, Field follows cached vector-field integration from perturbed source points, and Rain follows camera-footprint falling/surface-shedding routes with `feature_type = 4`.
...
- Viewport and raster export point shaders include sparse Ripple runtime effects, SAND Shoreline waves, compact-grid Seepage, Flow Trails, Rain Trails, and active-cloud Field Surface Motion without requiring water PLY export.
...
- Project serialization uses schema version 37 for the current project shape; standalone water-source documents use schema version 13.
...
- Rain settings include independent Vegetation Interception, Surface Runoff, and Splash switches; vegetation impacts drip/fade, runoff-off drops stop at mesh/support impact, and Splash adds capped rock-only secondary droplets.
