...
- Animation schema 12 stores normalized global water scenario keys with complete look snapshots, Seepage level/spread, continuous Rain and Flow levels, Mesh Flow level/Rain gain/persistence/rise/recession, Seepage Rain delay/rise/recession, outgoing Smooth/Linear/Hold interpolation, and embedded linked-scenario fallbacks. Each track can also store normalized per-node Activity, Local Spread, Wetting Front, Reach Scale, Width Scale, and Prominence keys by stable node ID. Animation-v10 `localSpread` remains readable; missing new factors default to one. The UI displays and edits key time in seconds while storage remains normalized. Water keys retain their timing when animation duration, frame rate, or camera keys change; copying water keying does not copy those animation properties.
- Scenario and look animation changes compact parameter buffers only. Each live or export sample resolves one immutable `WaterFrameState` containing normalized/sample time, the raw scenario, the Seepage-delayed scenario, the independently filtered Mesh Flow moisture value, and per-node states. Rain and ordinary Flow consume the raw state; Seepage uses its deterministic 120 Hz Delay/Rise/Recession moisture envelope, while Mesh Flow uses its own deterministic 120 Hz Rain rise/recession/persistence envelope. Visible Rain is not delayed and out-of-order scrubbing remains reproducible. Keyed Flow reveals a deterministic subset of prebuilt trails; keyed Mesh Flow changes active fixed-capacity particles and appearance without reallocating its Ground table, history, Rain-seed ring, or contact-event buffers. A long Rain/Flow/Seepage/Mesh Flow animation therefore does not rebuild connected Seepage support, rebuild Flow geometry, warm a triangle cache, dispatch CPU route generation, or upload point-cloud/shared-cache data.
...
- Project serialization uses schema version 45 for the current project shape; standalone water-source documents use schema version 19. Animation-path saves use schema version 12, nested Rain settings remain version 3, and the shared binary water-surface cache uses schema version 4.
...
- `Generated SampleScene validation project round-trips schema 45 without derived caches`
