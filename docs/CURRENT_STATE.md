...
- `--lod-compare` writes metrics schema v2 with source/cache fingerprints, density policy fields, deterministic selection and image hashes, plus a `beauty_adaptive_hq_error.exr` difference/error map beside the full-source and adaptive EXRs. Metrics include per-channel/RGB/luminance/alpha MAE/RMSE/max error, coverage and luminance ratios, representative counts, represented source counts, GPU pass timing, peak RSS, upload budget/chunk residency where available, and fallback reason fields.
...
- Adaptive export density-policy semantics, deterministic export traversal parameters, Match Viewport snapshot capture/rejection, Quick MP4 Fast Adaptive Preview labeling, Full Source exact-source rejection, and `--lod-compare` schema v2 deterministic fields/error-map metrics
...
- `lod_compare_metrics.json` should report `metrics_schema_version=2`, `density_policy`, `determinism`, source/cache fingerprints, image hashes, selection hashes, and `difference_exr`.
