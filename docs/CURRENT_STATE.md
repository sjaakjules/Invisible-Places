...
- `--lod-compare` writes metrics schema v3 with source/cache fingerprints, density policy fields, deterministic selection and image hashes, tile pressure/culling diagnostics, plus a `beauty_adaptive_hq_error.exr` difference/error map beside the full-source and adaptive EXRs. Metrics include per-channel/RGB/luminance/alpha MAE/RMSE/max error, coverage and luminance ratios, representative counts, represented source counts, GPU pass timing, peak RSS, upload budget/chunk residency where available, and fallback reason fields.
