...
- Animation schema 11 stores normalized global water scenario keys with complete look snapshots, Seepage level/spread, continuous Rain and Flow levels, Seepage Rain delay/rise/recession, outgoing Smooth/Linear/Hold interpolation, and embedded linked-scenario fallbacks. Each track can also store normalized per-node Activity, Local Spread, Wetting Front, Reach Scale, Width Scale, and Prominence keys by stable node ID. Animation-v10 `localSpread` remains readable; missing new factors default to one. The UI displays and edits key time in seconds while storage remains normalized. Water keys retain their timing when animation duration, frame rate, or camera keys change; copying water keying does not copy those animation properties.
- Scenario and look animation changes compact parameter buffers only. Each live or export sample resolves one immutable `WaterFrameState` containing normalized/sample time, the raw scenario, the Seepage-delayed scenario, and per-node states. Rain and Flow consume the raw state; only Seepage consumes the deterministic 120 Hz Delay/Rise/Recession moisture envelope, so visible Rain is not delayed and out-of-order scrubbing remains reproducible. Keyed Flow reveals a deterministic subset of prebuilt trails and scales their appearance/motion. A long Rain/Flow/Seepage animation therefore does not rebuild connected Seepage support, rebuild Flow geometry, dispatch route generation, or upload point-cloud/shared-cache data.
...
- Grouped ROCK/SAND/VEG scenes build combined Flow/Field analysis support only from CPU-ready canonical analysis sources, not from the selected display bundle. Separately, one shared `WaterSurfaceCache` streams every file in the exact complete 2 mm bundle once, or the nearest complete fallback bundle, and deterministically quantizes it into 10 mm Rain ROCK/SAND XY, VEG 3D, and orientation-independent authored-role support. ROCK/SAND retain the exact maximum height per XY cell while all normals are hemisphere-aligned before averaging. It does not load those sources as display or canonical analysis clouds. Display switching therefore leaves support signatures, connected Seepage topology, path/field caches, source placement, Rain collision, Flow guidance, and region analysis unchanged.
- Shared surface-cache schema 3 persists aggregate records, power-of-two GPU hash tables, and a streaming payload checksum at `<scene>/.invisible_places/cache/water/<signature>.surfacecache`, with project-local hidden fallback and a 5 GiB persistence ceiling. Schema-2 `.raincache` files are read only as migration inputs before a cold source scan. Runtime code uses the neutral `WaterSurfaceCache`/`WaterSurfaceSource`, `Build`/`Save`/`UploadWaterSurfaceCache`, `WaterSurfaceFlowView`, and `WaterSurfaceUploadRevision` API; the old Rain-cache runtime type aliases and public wrappers have been removed. The active scene warms only after its display upload completes; a valid disk cache loads sequentially without rescanning point clouds, rebuilding hashes, or hashing the full payload twice. One reusable staging allocation capped at 64 MiB copies the immutable tables into device-local memory, then edge-preserving normal/roughness preprocessing runs once. CPU GPU-table vectors are released after installation.
- Point-cloud loads, shared-surface build/load plus GPU preprocessing, and dynamic-mesh cache warmup share one exclusive high-memory slot. After the visible display commits, the shared surface cache gets the slot before inactive queued loads; other high-memory work resumes only after preprocessing completes.
...
- Project serialization uses schema version 43 for the current project shape; standalone water-source documents use schema version 18. Animation-path saves use schema version 11, and the shared binary water-surface cache remains schema version 3.
...
- Project documents also persist Water v2 authored state plus compact shared-surface and Flow-path manifests. Clean generated Flow branches and branch bake fingerprints live in `.flowpathcache`; generated cache arrays are not embedded in schema-43 JSON.
...
- Animation paths preserve water caustic look settings for current visual style compatibility and schema-11 normalized Seepage/Rain/Flow scenario tracks, nested per-node Seepage keys, and embedded scenario fallbacks for synchronized playback and export. Schema-10 Local Spread keys migrate with Reach Scale, Width Scale, and Prominence defaulting to one; missing Seepage Rain timing remains immediate. Older documents also default to full Flow (`flowLevel = 1`), visible global/source trails, source strength 1, Rain response 0, and disabled Surface Guide on legacy manual paths; newly authored manual paths enable Surface Guide. Existing saved looks without a pattern remain Legacy Ripples and unknown pattern names fall back to it.
...
- `Generated SampleScene validation project round-trips schema 43 without derived caches`
