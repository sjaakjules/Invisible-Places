...
- GPU Rain smoke coverage uses the approximately 2.1 million-point SampleScene and 20.7 million-point Scene1 3 mm display bundles. `rain-gpu-sample-scene` and `rain-gpu-scene1-3mm` assert that live weather/visual/effect edits and GPU EXR comparisons reuse one shared-surface upload. Cross-feature startup/edit/export coverage uses `water-integration-sample-scene` with `Saved/validation/SampleSceneValidation_project.json` and `water-integration-scene1-top-view` with `Saved/exhibitionScene_project.json`/`Top_View`.
