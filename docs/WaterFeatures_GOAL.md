...
```text
/goal Complete WaterFeatures_GOAL.md: Ripples affect only the base cloud, Flow and Field share one animated trail abstraction, Field vector caches are saved/reused offline, and all behavior is verified by focused tests plus full water regression checks.
```
...
- Ripples modify the active/base point cloud through composed `water_effect_*` fields only. Ripple calculations may use sparse in-memory effect points, but the active workflow must not create a visible `-Ripples.generated` particle/cloud session.
...
- Field regions can be drawn, converted into vector fields, saved to an offline cache, and reused when the source layer, region fingerprint, and field settings still match.
...
3. Base-Cloud Ripples
   ...
   - Preserve base-cloud `water_effect_*` composition and existing Ripple procedural differences.
   - Verify base Height/Intensity mappings still compose with Ripple and Field effects.
...
5. Field Cache Persistence
   - Save Field vector caches to `Saved/water/<source-stem>-WaterFieldCache.bin`.
   - Reuse caches when source, settings, and region fingerprints match.
