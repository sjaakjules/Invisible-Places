...
- Flow and Field create animated generated trail clouds through one shared trail representation and point-cloud scalar schema. Rain instead uses persistent GPU particles, the static 10 mm role-aware cache built from a complete 2 mm role bundle, and bounded impact-event lookups on the displayed point cloud.
...
- Preserve the active Water tabs: `Ripples`, `Shoreline`, `Rain`, `Flow`, and `Field`.
...
ctest --test-dir build/macos-debug -R "Water|Ripple|Shoreline|Rain|Field|Offline water" --output-on-failure
